      ...
      >
        ...
        <div className="flex items-center gap-3">
          {result.overallScore !== undefined && (
            <Badge variant={result.overallScore >= 3 ? "success" : "destructive"} className="text-xs font-input">
          ...
          {(evalCase?.channel || (evalCase?.contextParams && Object.keys(evalCase.contextParams).length > 0)) && (
            <div className="space-y-2">
              ...
              <div className="flex items-start gap-3 flex-wrap">
                {evalCase.channel && <Badge variant="outline" className="text-xs">{evalCase.channel}</Badge>}
...
interface EvalRunDetailSheetProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  runId: Id<"evalRuns"> | undefined
  agentId: string
}

export function EvalRunDetailSheet({ open, onOpenChange, runId, agentId }: EvalRunDetailSheetProps) {
  const run = useEvalRun(runId)
  const results = useEvalRunResults(runId)
  const effectiveSuiteId = run?.suiteId
  ...
  const handleRerunCase = useCallback(async (caseId: string) => {
    ...
    try {
      await startRun({ suiteId: run.suiteId, triggerSource: "dashboard", caseIds: [caseId as Id<"evalCases">] })
    } catch {
    } finally {
  ...
  const handleRunFailed = useCallback(async () => {
    ...
    try {
      ...
      await startRun({ suiteId: run.suiteId, triggerSource: "dashboard", caseIds: failedCaseIds })
    } catch {
    } finally {
      setStartingFailed(false)
    }
  }, [startRun, run, errorResults])

  const isRunning = run?.status === "pending" || run?.status === "running"
  const progressPct = run && run.totalCases > 0 ? (run.completedCases / run.totalCases) * 100 : 0

  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-3xl overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Run Results</SheetTitle>
        </SheetHeader>
          ...
          <div className="space-y-6 mt-4">
                ...
                {!isRunning && (
                  <>
                    {errorResults.length > 0 && (
                      <>
                        <button
                          ...
                          className="flex items-center gap-1.5 rounded-md bg-primary px-2.5 py-1.5 text-xs font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50 transition-colors ease-out-soft"
                        >
                          {startingFailed ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RotateCcw className="h-3.5 w-3.5" />}
                          Run Failed ({errorResults.length})
                        </button>
                        <button
                          onClick={handleCopyErrors}
                          className="flex items-center gap-1.5 rounded-md border border-destructive/30 px-2.5 py-1.5 text-xs text-destructive hover:bg-destructive/10 transition-colors ease-out-soft"
                        >
                          {copiedErrors ? <Check className="h-3.5 w-3.5 text-success" /> : <XCircle className="h-3.5 w-3.5" />}
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 rounded-md border px-2.5 py-1.5 text-xs text-content-secondary hover:bg-background-tertiary transition-colors ease-out-soft"
                    >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="rounded-md border bg-card p-3">
                ...
                <div className="text-base font-semibold font-display text-content-primary capitalize mt-0.5">{run.status}</div>
              </div>
              <div className="rounded-md border bg-card p-3">
                <div className="text-xs text-content-tertiary">Pass Rate</div>
                <div className="text-base font-semibold font-display text-content-primary mt-0.5">
