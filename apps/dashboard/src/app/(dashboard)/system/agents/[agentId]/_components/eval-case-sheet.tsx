...
import { Loader2, Plus, Trash2, Save } from "@/lib/icons"
import { useCreateEvalCase, useUpdateEvalCase, useEvalCase, useEvalSuite, useAgentWithConfig } from "@/hooks/use-convex-data"
...
export function EvalCaseSheet({ open, onOpenChange, suiteId, agentId, caseId }: EvalCaseSheetProps) {
  const createCase = useCreateEvalCase()
  const updateCase = useUpdateEvalCase()
  const evalCase = useEvalCase(caseId)
  const suite = useEvalSuite(suiteId)
  const agentData = useAgentWithConfig(suite?.agentId ?? agentId)
  ...
  useEffect(() => {
    if (open && !isEdit) {
      ...
      setDescription("")
      setTags("")
      setTurns([{ userMessage: "", assertions: [] }])
      setFinalAssertions([])
      setChannel("")
      setContextParams({})
      setError(null)
  ...
  const formatAssertions = (assertions: AssertionForm[]) =>
    assertions.map((a) => ({
      type: a.type,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const formattedTurns = turns.map((t) => ({
        ...
        assertions: t.assertions.length > 0 ? formatAssertions(t.assertions) : undefined,
      }))

      const formattedFinal = finalAssertions.length > 0 ? formatAssertions(finalAssertions) : undefined
      ...
      setError(err instanceof Error ? err.message : `Failed to ${isEdit ? "update" : "create"} case`)
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-2xl overflow-y-auto">
        <SheetHeader>
          <SheetTitle>{isEdit ? "Edit Test Case" : "New Test Case"}</SheetTitle>
        </SheetHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-6">
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : isEdit ? <Save className="h-4 w-4" /> : null}
