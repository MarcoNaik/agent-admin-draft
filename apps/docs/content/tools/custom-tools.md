...
| `struere` | `StruereSDK` | SDK object providing access to all built-in tools (entity, event, whatsapp, calendar, airtable, email, payment, agent). |
...
The `struere` parameter lets you call any built-in tool directly from a custom tool handler. This is useful for combining external API calls with platform operations like creating entities, emitting events, or sending messages.
...
```typescript
tools: ["entity.create", "entity.link", "event.emit", "whatsapp.send", "entity.query"]
```
