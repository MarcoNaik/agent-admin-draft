...
// .wrangler/tmp/bundle-4FgRjY/checked-fetch.js
...
// .wrangler/tmp/bundle-4FgRjY/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-4FgRjY/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-4FgRjY/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
var deployAgentSchema = external_exports.object({
  ...
  environment: external_exports.enum(["preview", "staging", "production"]).default("preview"),
...
// ../../node_modules/.bun/@clerk+backend@1.34.0+249a289e9906fbea/node_modules/@clerk/backend/dist/chunk-MQOIIRZU.mjs
var import_cookie = __toESM(require_dist7(), 1);
...
var clerkAuth = createMiddleware(async (c, next) => {
  ...
  try {
    ...
    const orgId = payload.org_id || payload.metadata?.organizationId;
    ...
    c.set("auth", {
      clerkUserId: userId,
      userId: payload.metadata?.internalUserId || userId,
      organizationId: orgId,
      email
    });
    await next();
...
var clerkWebhook = createMiddleware(async (c, next) => {
...
agentRoutes.get("/", async (c) => {
  const auth = c.get("auth");
  const db = createDb(c.env.DB);
  const result = await db.select().from(agents).where(and(
...
agentRoutes.get("/:id", async (c) => {
  ...
  const versions = await db.select().from(agentVersions).where(eq(agentVersions.agentId, agentId)).orderBy(desc(agentVersions.deployedAt)).limit(10);
...
deploymentRoutes.post("/agents/:agentId/deploy", async (c) => {
  ...
  const deploymentUrl = environment === "preview" ? `https://${agent.slug}-${deploymentId.slice(0, 8)}.${baseUrl}` : `https://${agent.slug}.${baseUrl}`;
  ...
  await db.update(agents).set({ currentVersionId: versionId, updatedAt: now }).where(eq(agents.id, agentId));
  return c.json({
    deployment: {
      id: deploymentId,
      agentId,
