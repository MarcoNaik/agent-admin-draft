import { existsSync, readFileSync, writeFileSync } from 'fs'
import { join } from 'path'

export interface StruereProject {
  agentId: string
  team: string
  agent: {
    slug: string
    name: string
  }
}

export interface StruereProjectV2 {
  version: '2.0'
  organization: {
    id: string
    slug: string
    name: string
  }
}

const PROJECT_FILE = 'struere.json'

export function loadProject(cwd: string): StruereProject | null {
  const projectPath = join(cwd, PROJECT_FILE)

  if (!existsSync(projectPath)) {
    return null
  }

  try {
    const data = readFileSync(projectPath, 'utf-8')
    return JSON.parse(data) as StruereProject
  } catch {
    return null
  }
}

export function loadProjectV2(cwd: string): StruereProjectV2 | null {
  const projectPath = join(cwd, PROJECT_FILE)

  if (!existsSync(projectPath)) {
    return null
  }

  try {
    const data = readFileSync(projectPath, 'utf-8')
    const parsed = JSON.parse(data)

    if (parsed.version === '2.0') {
      return parsed as StruereProjectV2
    }

    return null
  } catch {
    return null
  }
}

export function saveProject(cwd: string, project: StruereProject): void {
  const projectPath = join(cwd, PROJECT_FILE)
  writeFileSync(projectPath, JSON.stringify(project, null, 2) + '\n')
}

export function saveProjectV2(cwd: string, project: StruereProjectV2): void {
  const projectPath = join(cwd, PROJECT_FILE)
  writeFileSync(projectPath, JSON.stringify(project, null, 2) + '\n')
}

export function hasProject(cwd: string): boolean {
  return existsSync(join(cwd, PROJECT_FILE))
}

export function isV2Project(cwd: string): boolean {
  const project = loadProjectV2(cwd)
  return project !== null
}

export function getProjectVersion(cwd: string): '1.0' | '2.0' | null {
  const projectPath = join(cwd, PROJECT_FILE)

  if (!existsSync(projectPath)) {
    return null
  }

  try {
    const data = readFileSync(projectPath, 'utf-8')
    const parsed = JSON.parse(data)

    if (parsed.version === '2.0') {
      return '2.0'
    }

    if (parsed.agentId) {
      return '1.0'
    }

    return null
  } catch {
    return null
  }
}
